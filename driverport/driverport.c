...
static ssize_t driverport_write(struct file *pfile, const char *poctets, size_t nb,loff_t *pos) {
  ...
  if (convert_result > 255){
    ...
    //Setting gpio output values based on mapped GPIOs. Little-endian.
    for (init_gpio_loop = 0; init_gpio_loop < 8; init_gpio_loop++){
      #ifdef DEBUG
...
//Little-Endian: Read the least-significant byte first 
...
MODULE_DESCRIPTION("driver 8 leds pour pmodhat JC");
